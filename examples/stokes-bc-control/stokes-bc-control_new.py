...
# algorithm.
c = g.copy(deepcopy=True)
h = g.copy(deepcopy=True)
from numpy.random import rand
h.vector()[:] = rand(V.collapse().dim())

dJdm = compute_gradient(J, m)
H = Hessian(J, m)
Hm = H(dJdm)

#print taylor_test(Jhat, c, dJdm, dJdm=dJdm.vector().inner(dJdm.vector()), Hm=Hm.vector().inner(dJdm.vector()))

#u, p = s.split(True)

file = File("stokes-hessian.pvd")
file << u
