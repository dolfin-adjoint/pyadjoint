...
class Function(OverloadedType, backend.Function):
    ...
    def split(self, deepcopy=False, **kwargs):
        ...
        if not annotate:
            ...
            ret = tuple(compat.create_function(self,
                                               i,
                                               _ad_block_class=FunctionSplitBlock,
                                               _ad_floating_active=True,
                                               _ad_args=[self, i],
                                               _ad_output_args=[i],
                                               _ad_output_block_class=FunctionMergeBlock,
                                               _ad_outputs=[self])
                        for i in range(num_sub_spaces))
        return ret
    ...
    def _ad_create_checkpoint(self):
        ...
        dep = self._ad_block.get_dependencies()[0].saved_output
        return dep.sub(self._ad_block.idx, deepcopy=False)
