...
def test_cahn_hilliard():
    ...
    # This actually works even though assign is done below because the assign is done with annotate=False.
    m = Control(u0)
    ...
    dJdm = h._ad_dot(dJdic)
    Hic = Hessian(J, m)
    Hm = h._ad_dot(Hic(h))

    minconv = taylor_test(ReducedFunctional(J, m), Function(ME, ic.vector()), h, dJdm=dJdm, Hm=Hm)
    assert minconv > 2.9